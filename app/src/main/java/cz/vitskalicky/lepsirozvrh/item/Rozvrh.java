...
public class Rozvrh {
    ...
    private String cycleShort;
    private String cycleName;

    private int lessonsCount;
    private RozvrhLessonCaption[] lessonCaptions;

    private int dayCount;

    /**
     * Creates rozvrh from an xml document
     */
    public Rozvrh(Document document){
        try{
            Element results = document.getDocumentElement();
            results.normalize();

            Node rozvrh = results.getElementsByTagName("rozvrh").item(0);
            NodeList rozvrhChildNodes = rozvrh.getChildNodes();

            //first, we need to parse lesson captions, because lessons rely on them
            Node hodiny = results.getElementsByTagName("hodiny").item(0);
            NodeList lessonCaptionsC = hodiny.getChildNodes();
            lessonsCount = Integer.parseInt(hodiny.getFirstChild().getTextContent());
            lessonCaptions = new RozvrhLessonCaption[lessonsCount];

            for (int i = 1; i < lessonsCount + 1 ; i++) {
                Node item = lessonCaptionsC.item(i);
                lessonCaptions[i] = new RozvrhLessonCaption(item);
            }

            for (int i = 0; i < rozvrhChildNodes.getLength(); i++) {
                Node item = rozvrhChildNodes.item(i);
    ...
    public RozvrhLessonCaption[] getLessonCaptions() {
        return lessonCaptions;
    }
